...
while (now > 1496275200):
    #161049802 corresponds to Zagitova
    url = "https://www.sports.ru/stat/tags/player/lenta/161049802.html?ts=" + str(int(now)) + "&filters[2]=all&filters[3]=news&no_controls=1"
    ...
    if len(links) > 0:
        ...
        for j in range(len(links)):
            ...
            tx = links[j].text
            #print(tx);
            with open('news_list_zagi.txt', 'a', encoding='utf-8') as file:
                file.write(link + "; " + tx + "\n")
    else:
        print("0")
        now += 12 * 60 * 60
